...
for i in range(df3.shape[1]-1):
    pvalues.append(stats.mannwhitneyu(
        *[group[df3.columns[i]].values for name, group in df3.groupby(df3.iloc[:, -1])]).pvalue)
    print(str(df3.columns[i]) + ": " + str(pvalues[-1]))
    print(str(df3.columns[i]) + ": " + str(pvalues[-1]*n_var))

cohend_values_clusters = []
for i in range(df3.shape[1]-1):
    cohend_values_clusters.append(-cohend(
        *[group[df3.columns[i]].values for name, group in df3.groupby(df3.iloc[:, -1])]))
    print(str(df3.columns[i]) + ": " + str(cohend_values_clusters[-1]))

df3.groupby("cluster").mean().T
df3.groupby("cluster").std().T

df_cohend_clusters = pd.DataFrame(
    DataForCorrelationPCA.columns, columns=["variable"])
...
with sns.axes_style("darkgrid"):
    ...
    g = sns.swarmplot(ax=ax1, data=df3_long, x="variable", y="value",
                      hue="cluster", dodge="hue", size=3, palette=["blue", "green"])
    textColors = [("red" if i < 0.05/n_var else ("blue" if i <
                   0.05 else "black")) for i in pvalues]
    for i in range(n_var):
        ax1.text(x=i, y=0.8 * ax1.get_ylim()[1], s="{:.2e}".format(
            pvalues[i]), rotation=90, color=textColors[i], va="center", ha="center")
    ax1.set_xlabel(None)
    ...
    sns.barplot(ax=ax2, data=df_cohend_clusters, x="variable",
                y="Cohens' d", palette=barcols, alpha=.5)
    ax2.set_xlabel(None)
    ax2.set_xticklabels([])
    ...
    mosaic(dfClusterOlfdiag, ["cluster", "Olfdiag"],
           gap=0.05, properties=props, ax=ax3)
    ax3.set_xlabel("Cluster")
    ax3.set_ylabel("Diagnosis")

    contigency_1 = pd.crosstab(
        dfClusterOlfdiag['Olfdiag'], dfClusterOlfdiag["cluster"])
    contigency_1
...
contigency_3 = pd.crosstab(
    dfClusterOlfdiagSex['sex_0f'], dfClusterOlfdiagSex["cluster"])
contigency_3
c, p, dof, expected = chi2_contingency(contigency_3)

contigency_5 = pd.crosstab(
    dfClusterOlfdiagSex['Olfdiag'], dfClusterOlfdiagSex["sex_0f"])
contigency_5
...
for i in range(df4.shape[1]-1):
    cohend_values_olfdiag.append(-cohend(
        *[group[df4.columns[i]].values for name, group in df4.groupby(df4.iloc[:, -1])]))
    print(str(df4.columns[i]) + ": " + str(cohend_values_olfdiag[-1]))

df_cohend_clusters_olfdiag = df_cohend_clusters.copy()
df_cohend_clusters_olfdiag.rename(
    columns={"Cohens' d": "d Cluster"}, inplace=True)
df_cohend_clusters_olfdiag["d OlfDiag"] = cohend_values_olfdiag
df_cohend_clusters_olfdiag["d_cluster_not_olfdiag"] = abs(
    df_cohend_clusters_olfdiag["d Cluster"]) - abs(df_cohend_clusters_olfdiag["d OlfDiag"])

df_cohend_ABC_d_cluster_not_olfdiag = df_cohend_clusters_olfdiag[[
    "variable", "d_cluster_not_olfdiag"]]
df_cohend_ABC_d_cluster_not_olfdiag .set_index("variable", inplace=True)
df_cohend_ABC_d_cluster_not_olfdiag .sort_values(
    by="d_cluster_not_olfdiag", inplace=True, ascending=False)
...
with sns.axes_style("darkgrid"):
    ...
    barcols = ["dodgerblue" if (i) < ABC_A_varimportance_d_cluster_not_olfdiag["ABlimit"]
               else "blue" for i in df_cohend_ABC_d_cluster_not_olfdiag["d_cluster_not_olfdiag"]]
    sns.barplot(ax=ax2, data=df_cohend_ABC_d_cluster_not_olfdiag, y=df_cohend_ABC_d_cluster_not_olfdiag.index.tolist(
    ), x="d_cluster_not_olfdiag", palette=barcols, alpha=1)
    ax2.set_title("Difference of effect sizes")
    ax2.set_xlabel(
        "|Cohen's d for clusters| - |Cohen's d for olfactory diagnosis|")
