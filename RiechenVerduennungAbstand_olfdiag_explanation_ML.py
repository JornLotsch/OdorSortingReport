...
if len(ABC_A_FS_sumscore_nested["Aind"]) > 0:
    sparse_feature_names = ABC_A_FS_sumscore_nested["Aind"].index.tolist()
else:
    sparse_feature_names = ABC_A_FS_sumscore_nested["Bind"].index.tolist()
...
dfFS_sumscore_diff = pd.DataFrame(
    FS_sumscore_diff.index.tolist(), columns=["variable"])
...
with sns.axes_style("darkgrid"):
    ...
    barcols = ["dodgerblue" if (
        i) < 6 else "blue" for i in dfFS_sumscore_diff["diff"].to_numpy()]
    sns.barplot(ax=ax1, x=dfFS_sumscore_diff.index.tolist(),
                y=dfFS_sumscore_diff["diff"], palette=barcols, alpha=1)
    ax1.set_title(
        "Difference 'cluster - olfactory diagnosis' in the number of selections by 17 different methods")
    ax1.set_xlabel(None)
